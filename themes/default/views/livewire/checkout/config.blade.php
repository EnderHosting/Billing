                    ...
                    @if ($prices->type == 'recurring')
                        <div class="mt-4">
                            ...
                            <div class="flex flex-row flex-wrap mt-2 mb-4 gap-4">
                                @php 
                                    $priceTypes = ['weekly', 'monthly', 'quarterly', 'semi_annually', 'annually', 'biennially', 'triennially']; 
                                    $cycleLengths = [
                                        'weekly' => 1 / 4, // Ajusta el ciclo semanal para que sea 1/4 de un mes
                                        'monthly' => 1,
                                        'quarterly' => 3,
                                        'semi_annually' => 6,
                                        'annually' => 12,
                                    ...
                                    // Verifica si el precio mensual es 0
                                    $monthlyPrice = $monthlyPrice ?: 1; // Evitar división por cero al establecer un valor predeterminado
                                @endphp
                                @foreach ($priceTypes as $priceType)
                                    @if ($prices->{$priceType})
                                        @php
                                            $price = $prices->{$priceType};
                                            $originalPrice = $monthlyPrice * $cycleLengths[$priceType];
                                            $savingsPercentage = $originalPrice > 0 ? round((($originalPrice - $price) / $originalPrice) * 100) : 0;
                                        @endphp
                                        <button type="button"
                                            ...
                                            <div class="flex items-baseline space-x-1">
                                                ...
                                                @if ($priceType != 'monthly' && $priceType != 'weekly')
                                                    <div class="text-xs align-top" style="color: #FFD700;">
                                                        <pre> {{ $savingsPercentage > 0 ? "SAVE $savingsPercentage%" : '' }} </pre>
                                                    </div>
                                                @endif
                                            </div>
