...
namespace App\Extensions\Gateways\PayPal_IPN;
    ...
    public function webhook()
    {
        ...
        if ($res == 'VERIFIED') {
            if ($myPost['payment_status'] == 'Completed') {
                if ($myPost['mc_gross'] == number_format(Invoice::findOrfail($myPost['custom'])->total(), 2, '.', '') && $myPost['mc_currency'] == ExtensionHelper::getCurrency()) {
                    ExtensionHelper::paymentDone($myPost['custom'], 'PayPal IPN', $myPost['txn_id']);
                }
            }
        }
