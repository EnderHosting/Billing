...
class Index extends Component
{
    ...
    public $selectedPricing = 'monthly'; // Default selection
    ...
    public function products()
    {
        ...
        if ($cart) {
            foreach ($cart as $key => $product2) {
                ...
                $product->billing_cycle = $product2['billing_cycle'] ?? $this->selectedPricing;
                $product->setup_fee = $product2['setup_fee'] ?? 0;

                // Establecer el precio y la tarifa de instalación según el ciclo de facturación seleccionado
                switch ($product->billing_cycle) {
                    case 'weekly':
                        $product->price = $product->prices->weekly;
                        $product->setup_fee = $product->prices->weekly_setup;
                        break;
                    case 'monthly':
                        $product->price = $product->prices->monthly;
                        $product->setup_fee = $product->prices->monthly_setup;
                        break;
                    case 'quarterly':
                        $product->price = $product->prices->quarterly;
                        $product->setup_fee = $product->prices->quarterly_setup;
                        break;
                    case 'semi_annually':
                        $product->price = $product->prices->semi_annually;
                        $product->setup_fee = $product->prices->semi_annually_setup;
                        break;
                    case 'annually':
                        $product->price = $product->prices->annually;
                        $product->setup_fee = $product->prices->annually_setup;
                        break;
                    case 'biennially':
                        $product->price = $product->prices->biennially;
                        $product->setup_fee = $product->prices->biennially_setup;
                        break;
                    case 'triennially':
                        $product->price = $product->prices->triennially;
                        $product->setup_fee = $product->prices->triennially_setup;
                        break;
                }
                ...
                if (isset($coupon->products)) {
                    if (!in_array($product->id, $coupon->products) && !empty($coupon->products)) {
                        ...
                        if ($coupon->type == 'percent') {
                            ...
                            $product->discount_fee = $coupon->setup_fee * $coupon->value / 100;
                        } else {
    ...
    private function createOrderProduct(Order $order, Product $product, Invoice $invoice, $setQuantity = true)
    {
        ...
        if ($product->billing_cycle) {
            ...
            if ($product->billing_cycle == 'weekly') {
                $orderProduct->expiry_date = date('Y-m-d H:i:s', strtotime('+1 week'));
            } elseif ($product->billing_cycle == 'monthly') {
