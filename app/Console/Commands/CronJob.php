...
use App\Models\Log as AppLog; // Alias para evitar conflicto
use App\Models\OrderProduct;
use App\Models\OrderProductUpgrade;
use Illuminate\Support\Facades\Http;
use Illuminate\Support\Facades\Log; // Logger de Laravel
...
class CronJob extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($orders as $order) {
            ...
            if ($order->status == 'paid') {
                ...
                if ($order->expiry_date && strtotime($order->expiry_date) < strtotime('-' . config('settings::remove_unpaid_order_after', 7) . ' days')) {
                    ExtensionHelper::terminateServer($order);
                    $order->status = 'cancelled';
        ...
        foreach (OrderProductUpgrade::with('orderProduct')->get() as $orderProductUpgrade) {
            if ($orderProductUpgrade->orderProduct && $orderProductUpgrade->orderProduct->expiry_date && $orderProductUpgrade->orderProduct->expiry_date < now()) {
                $orderProductUpgrade->delete();
            } else {
                if ($orderProductUpgrade->orderProduct) {
                    if ($orderProductUpgrade->invoice) {
                        // Update the price
                        ...
                        // Manejo del caso donde invoice es null
                        Log::debug('Invoice es null en OrderProductUpgrade ID: ' . $orderProductUpgrade->id);
                    }
                } else {
                    // Manejo del caso donde orderProduct es null
                    Log::debug('OrderProduct es null en OrderProductUpgrade ID: ' . $orderProductUpgrade->id);
                }
            }
        }
        ...
        $this->info('Deleted Logs: ' . AppLog::where('created_at', '<', now()->subDays(7))->count());
        AppLog::where('created_at', '<', now()->subDays(7))->delete();
    ...
    private function calculateAmount($product, $orderProduct)
    {
        ...
        if ($orderProduct && $orderProduct->billing_cycle && isset($cycleToDays[$orderProduct->billing_cycle])) {
            $amount = $product->price($orderProduct->billing_cycle) - 
                ($orderProduct->product->price($orderProduct->billing_cycle) / $cycleToDays[$orderProduct->billing_cycle] * 
                ($orderProduct->expiry_date ? $orderProduct->expiry_date->diffInDays() : 0));

            return $amount;
        }

        return 0; // Devuelve 0 o un valor por defecto si orderProduct es null o billing_cycle no está definido
    }
}